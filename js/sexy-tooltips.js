...
var ToolTip = new Class({
    ...
    create: function() {
        ...
        if (this.options.sticky) {
            this.close = new Element('a', { 'class': 'tooltip-close' }).injectInside(this.skeleton.top_left);
        }
    ...
    hide: function (event) {
        this.tooltip.set('morph', $extend(this.options, {onComplete: function() {this.fireevents(2)}.bind(this)}));
        this.tooltip.morph({
            'opacity': 0,
    ...
    position: function (event) {
        ...
        var doc  = $(document).getCoordinates();
        var top  = 0;
        var left = 0;

        var doc = $extend(doc, $(document).getScroll());
